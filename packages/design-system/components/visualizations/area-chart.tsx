...
import { useOnWindowResize } from '@d21/design-system/hooks/useOnWindowResize';
import {
  AvailableChartColors,
  type AvailableChartColorsKeys,
  constructCategoryColors,
  getColorClassName,
} from '@d21/design-system/lib/chart';
import { cn } from '@d21/design-system/lib/utils';
import { getYAxisDomain } from '@d21/design-system/lib/y-axis';

function hasOnlyOneValueForKey(data: Record<string, unknown>[], key: string): boolean {
  return data.every((item, _, arr) => item[key] === arr[0][key]);
}
...
const LegendItem = ({ name, color, onClick, activeLegend }: LegendItemProps) => {
  ...
  return (
    <button
      ...
      <p
        className={cn(
          "whitespace-nowrap text-description",
...
const ChartLegend = (
  { payload }: { payload: RechartsLegendProps['payload'] | undefined },
  ...
  return (
    <div
      ...
      <Legend
        categories={legendPayload.map((entry) => String(entry.value))}
...
type CurveClickProps = CurveProps & {
...
const AreaChart = React.forwardRef<HTMLDivElement, AreaChartProps>((props, ref) => {
  ...
  function onDotClick(itemData: DotClickData, event: React.MouseEvent) {
    ...
    <div ref={ref} className={cn("h-80 w-full", className)} {...other}>
      <ResponsiveContainer>
        <RechartsAreaChart
          ...
          {showGridLines ? (
            <CartesianGrid
              className={cn('stroke-1 stroke-neutral-200 dark:stroke-neutral-800')}
              horizontal={true}
              vertical={false}
            />
          ) : null}
          <XAxis
            ...
            ticks={startEndOnly ? [data[0][index], data[data.length - 1][index]] as (string | number)[] : undefined}
            fill=""
            stroke=""
            className={cn("fill-neutral-500 dark:fill-neutral-500")}
            tickLine={false}
            axisLine={false}
            minTickGap={tickGap}>
            {xAxisLabel && (
              <Label
                ...
                offset={-20}
                className='fill-neutral-800 dark:fill-neutral-200 font-medium'>
                {xAxisLabel}
              </Label>
            )}
          </XAxis>
          ...
          <Tooltip
            ...
            cursor={{ stroke: "#96989C", strokeWidth: 1 }}
            offset={16}
            position={{ y: 0 }}
            content={(props: { active?: boolean; payload?: TooltipPayload[]; label?: string }) => {
              const { active, payload, label } = props;
              //
              const cleanPayload: PayloadItem[] = payload?.filter((item) => item.type !== "none").map((item: TooltipPayload) => ({
              ...
              if (tooltipCallback && (active !== prevActiveRef.current || label !== prevLabelRef.current)) {
                tooltipCallback({ active, payload: cleanPayload, label: label ?? '' });
            ...
            return (
              <React.Fragment key={category}>
                <defs key={"defs-" + category}>
                  <linearGradient
                    key={"line-gradient-" + category}
                    className={cn(
                      getColorClassName(categoryColors.get(category) as AvailableChartColorsKeys, "text")
                    )}
                    id={categoryId}
                    x1="0"
                    y1="0"
                ...
                <Area
                  className={cn('focus:outline-none',
                    getColorClassName(categoryColors.get(category) as AvailableChartColorsKeys, "stroke")
                  )}
                  ...
                  activeDot={(props: any) => {
                    ...
                    return (
                      <Dot
                        ...
                        cy={cyCoord}
                        r={5}
                        fill={getColorClassName(categoryColors.get(dataKey) as AvailableChartColorsKeys, "fill")}
                        stroke={getColorClassName(categoryColors.get(dataKey) as AvailableChartColorsKeys, "stroke")}
