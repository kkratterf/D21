...
const tooltipStyles = tv({
  base: [
    // base
    'text-sm z-50 font-medium max-w-60 select-none rounded-lg px-2 py-1 leading-5 shadow-sm',
    // transition
  ...
  variants: {
    variant: {
      outline: 'bg-elevated text border border-default',
      fill: 'bg-inverse text-inverse',
    },
  },
  defaultVariants: {
    variant: 'outline',
          ...
          <TooltipPrimitives.Content
            ...
            {showArrow ? (
              <TooltipPrimitives.Arrow
                className={cn(
                  "border-none",
                  variant === "fill" ? "bg-inverse" : "bg-elevated border-default"
                )}
                width={12}
                height={7}
                aria-hidden="true"
