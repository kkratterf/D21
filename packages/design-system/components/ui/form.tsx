...
import { Label } from "@d21/design-system/components/ui/label";
  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <FormItemContext.Provider value={{ id }}>
        <div ref={ref} className={cn("text-sm space-y-2.5", className)} {...props} />
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <Label
      ref={ref}
      className={cn(error && "text-danger text-sm", className)}
      htmlFor={formItemId}
      {...props}
    />
  );
...
const FormDescription = React.forwardRef<HTMLParagraphElement, React.HTMLAttributes<HTMLParagraphElement>>(
  ({ className, ...props }, ref) => {
    ...
    return (
      <p ref={ref} id={formDescriptionId} className={cn("text-description text-sm", className)} {...props} />
    );
  }
);
