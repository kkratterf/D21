...
>(({ className, hint, shortcut, children, ...props }, forwardedRef) => (
  <ContextMenuPrimitive.RadioItem
    ...
    <span className="left-3 absolute flex justify-center items-center size-2">
      <Circle
        aria-hidden="true"
        className="group-data-[state=unchecked]/ContextMenuRadioItem:hidden group-data-[state=checked]/ContextMenuRadioItem:flex fill-current size-full text-brand shrink-0"
      />
    </span>
    {children}
    {hint && <span className={cn("text-description ml-auto pr-1 text-sm")}>{hint}</span>}
